...
const button={},minuteButton={},calls=[];
const ctx={document:{querySelectorAll:()=>[minuteButton]},state:{data:{result_refresh:{status:'failed'}}},q:()=>button,api:async name=>{calls.push(name);return {}},notice(){}};
...
(async()=>{await button.onclick();assert.deepEqual(calls,['retry_failed_results']);
 ...
 assert.equal(minuteButton.disabled,true);
 ctx.renderRefreshControls({status:'partial_failure',failure_count:1});assert.equal(minuteButton.disabled,false);
 assert.equal(minuteButton.textContent,'补取缺失行情');

...
const SHAQAccounts = (() => {
  ...
  function officialOutcome(trade) {
    if (trade.official_open == null || trade.official_close == null) {
      return '官方 O→C 方向成绩：等待独立核验';
    }
    return `官方 O→C ${usd(trade.official_open)} → ${usd(trade.official_close)} · 方向成绩 ${trade.direction_correct ? '正确' : '错误'}`;
  }
  ...
  function plot(accounts) {
    ...
    if (!points.length) return '<p>收盘净值：首个最终确认的持续账户交易日后显示曲线。</p>';
  ...
  function resultRows(rows, versions) {
    return (rows || []).map(row => `<tr class="${row.batch_id ? 'clickable' : ''}" ${row.batch_id ? `data-account-batch="${e(row.batch_id)}" data-account-key="${e(row.variant_key)}"` : ''}>
      ...
      <td>${e(statusName(row.status))}</td><td>${usd(row.net_pnl)}</td><td>${usd(row.gross_pnl)}</td>
      <td>${usd(row.fees)}</td><td>${usd(row.slippage_cost)}</td></tr>`).join('');
  }
  ...
  function overviewHtml(data, versions, filters={}) {
    ...
    return `<h2>独立分钟回放账户</h2><p>${rulesText(data.rules)}</p>
      <p class="account-note">收盘后模拟回放。方法、模型、引擎和规则各自绑定账户；暂定/未完成不会提前进入最终净值。卖空仅作可借券研究假设。</p>${plot(accounts)}
      ...
      <h3>持续账户每日记录</h3><div class="account-scroll"><table class="table"><thead><tr><th>日期 / 方法</th><th>范围</th><th>状态</th><th>净盈亏</th><th>零成本盈亏</th><th>手续费</th><th>滑点影响</th></tr></thead><tbody>${resultRows(forward, versions) || '<tr><td colspan="7">尚无持续账户记录。</td></tr>'}</tbody></table></div>
      <details class="account-secondary"><summary>历史 / 练习（不入账）</summary><div class="account-scroll"><table class="table"><thead><tr><th>日期 / 方法</th><th>范围</th><th>状态</th><th>净盈亏</th><th>零成本盈亏</th><th>手续费</th><th>滑点影响</th></tr></thead><tbody>${resultRows(research, versions) || '<tr><td colspan="7">没有历史或练习回放。</td></tr>'}</tbody></table></div></details>

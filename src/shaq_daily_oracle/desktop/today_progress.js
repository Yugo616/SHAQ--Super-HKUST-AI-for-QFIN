...
const SHAQProgress = (() => {
  ...
  function researchHtml(events, legacyReports, selection={}) {
    ...
    const taskKey=r=>r.call_id?`call:${r.call_id}:${r.attempt||1}`:r.stage==='report_validated'?`report:${r.symbol}:${r.domain}`:`stage:${r.stage}:${r.domain||''}`;
    const tasks=new Map();for(const row of relevant)tasks.set(taskKey(row),row);
    const completed=[...tasks.values()].filter(r=>['complete','cache_hit','validated','no_data','reused'].includes(r.status)||['report_validated','decision_complete','variant_reused'].includes(r.stage)).length;
    const times=relevant.map(r=>Date.parse(r.occurred_at_et)).filter(Number.isFinite), elapsed=times.length>1?(Math.max(...times)-Math.min(...times))/1000:Math.max(0,...relevant.map(r=>Number(r.elapsed_seconds||0)));
    const requested=new Set(relevant.filter(r=>r.stage==='call_requested').map(taskKey)), settled=new Set(relevant.filter(r=>['cache_hit','model_returned','failure'].includes(r.stage)).map(taskKey));
    const inflight=[...requested].filter(key=>!settled.has(key)).length;
    ...
    const timeline=relevant.map(r=>`<li>${esc(labels[r.stage]||r.stage)} · ${esc(r.domain||'')} ${esc((r.symbols||[r.symbol]).filter(Boolean).join('、'))} · ${esc(r.status||'')} · ${esc(r.occurred_at_et||'')}${r.elapsed_seconds!=null?` · ${Number(r.elapsed_seconds).toFixed(1)}秒`:''}</li>`).join('');
    return `<section class="research-view"><label>版本 <select data-research-variant>${variants.map(v=>option(v,chosen,'data-research-variant-option')).join('')}</select></label><label>候选 <select data-research-symbol>${symbols.map(s=>option(s,symbol,'data-research-symbol')).join('')}</select></label><p>实际任务 ${tasks.size} · 已完成 ${completed} · 进行中 ${inflight} · 实际耗时 ${elapsed.toFixed(1)} 秒</p><details><summary>执行时间线</summary><ol>${timeline}</ol></details>${sections||'<p>尚无已校验报告；原始无效输出不会显示为结论。</p>'}</section>`;
  }

...
const SHAQProgress = (() => {
  ...
  function researchHtml(events, legacyReports, selection={}) {
    ...
    const taskKey=r=>r.stage==='report_validated'?`report:${r.symbol}:${r.domain}`:r.call_id?`call:${r.call_id}:${r.attempt||1}`:`stage:${r.stage}:${r.domain||''}`;
    ...
    const requested=new Set(relevant.filter(r=>r.stage==='call_requested').map(taskKey)), settled=new Set(relevant.filter(r=>['cache_hit','model_returned','failure','validation_failure'].includes(r.stage)).map(taskKey));

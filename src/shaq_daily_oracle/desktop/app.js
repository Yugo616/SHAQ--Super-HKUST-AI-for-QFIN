...
function renderUpload(){const s=state.data.settings,drafts=state.data.drafts||[],allowed=s.github_upload_allowed===true;const options=drafts.map(d=>`<option value="${esc(d.draft_id)}">${esc(d.draft_id)} · ${esc(Object.keys(d.skill_hashes||{}).map(x=>x.split('/')[1]).filter((x,i,a)=>a.indexOf(x)===i).join('、'))}</option>`).join('');q('#upload').innerHTML=`<div class="card upload-single"><h2>上传 Shadow 版本</h2><p>选择一个本地草稿；本次改过的每个领域都会连同方法、研究依据和角色卡一起上传。</p><label>选择本地草稿<select id="upload-id">${options||'<option value="">还没有本地草稿</option>'}</select></label><label>一句研究说明<textarea id="upload-description" rows="4" placeholder="例如：补充事件意外程度的可核验判断，并给出对应研究依据。"></textarea></label><button class="primary" id="upload-button" ${allowed&&drafts.length?'':'disabled'}>上传版本</button><p class="inline-status">${allowed?'准备好后即可上传。':'当前账号为只读；仍可下载团队版本到本地。'}</p></div>`;q('#upload-button').onclick=async()=>{try{const v=await api('upload_skill_draft',q('#upload-id').value,q('#upload-description').value.trim());notice(`已上传到 ${v.branch} · ${v.commit_sha.slice(0,12)}`);await load();showPage('upload')}catch(e){notice(e.message,true)}}}
...
renderBatch=function(batch,key,symbol){renderBatchWithDefaultCandidate(batch,key);if(symbol)window.showCandidate(batch.batch_id,key,symbol)};
...
function bindSetup() {
  ...
  researchForm.onsubmit=async event=>{
    ...
    try {
      await api('save_lab_setup',{sec_identity:new FormData(event.target).get('sec_identity')});
      await load(false);q('#setup').classList.remove('hidden');
      q('#research-status').textContent='身份已保存。请完成环境检查。';
    } catch(error) {q('#research-status').textContent=error.message;}
  };
  q('#check-research-environment').onclick=checkResearchEnvironment;
  applyProtocolPreset();
}

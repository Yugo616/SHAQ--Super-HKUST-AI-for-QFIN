...
function teamSync(){openUtility('upload','团队同步');const bar=document.createElement('div');bar.className='editor-tabs';bar.innerHTML='<button class="secondary" id="sync-upload">上传版本</button><button class="secondary" id="sync-download">下载版本</button>';q('#utility-content').prepend(bar);q('#sync-upload').onclick=()=>teamSync();q('#sync-download').onclick=()=>{q('#upload').classList.remove('active');document.querySelector('main').append(q('#upload'));q('#utility-content').append(q('#updates'));q('#updates').classList.add('active');renderUpdates()}}
renderRun=function(){

...
function plotResults(rows){
  ...
  const step=Math.max(.01,10**Math.floor(Math.log10(Math.max(hi-lo,1)/4)));
  lo=Math.floor(lo/step)*step;hi=Math.ceil(hi/step)*step;if(hi===lo)hi=lo+step;
  const dates=[...new Set(all.map(r=>r.trade_date))].sort();
  const x=r=>110+dates.indexOf(r.trade_date)/Math.max(1,dates.length-1)*730;
  const y=v=>150-(v-lo)/(hi-lo)*125;
  const colors=['#285ba8','#35846c','#ad7833','#89559c','#7c8289'];
  const ticks=Array.from({length:5},(_,i)=>lo+(hi-lo)*i/4);
  return '<p>一股零费用累计盈亏 · 美元（不是账户余额）</p>'+ `<svg class="pnl-chart" viewBox="0 0 900 190" role="img" aria-label="各版本累计纸面盈亏">${ticks.map(v=>`<line x1="105" x2="850" y1="${y(v)}" y2="${y(v)}" stroke="#e3e6eb"/><text x="95" y="${y(v)+4}" text-anchor="end">$${v.toFixed(2)}</text>`).join('')}${series.map((rs,i)=>`<polyline fill="none" stroke="${colors[i%colors.length]}" stroke-width="2" points="${rs.map(r=>`${x(r)},${y(Number(r.cumulative_pnl||0))}`).join(' ')}"/>${rs.map(r=>`<circle cx="${x(r)}" cy="${y(Number(r.cumulative_pnl||0))}" r="3" fill="${colors[i%colors.length]}"/>`).join('')}`).join('')}<text x="110" y="180">${esc(dates[0])}</text><text x="840" y="180" text-anchor="end">${esc(dates.at(-1))}</text></svg><div class="chart-legend">${series.map((rs,i)=>`<p style="color:${colors[i%colors.length]}">● ${esc(historyMethod(rs[0]))} · ${esc(rs[0].trade_date)}起</p>`).join('')}</div>`;
}
...
renderEditor=function(){editorWithLayout();const first=document.createElement('button');first.className='module-button';first.textContent='候选筛选';first.dataset.module='screening';q('.module-menu').prepend(first);const panel=document.createElement('div');panel.id='module-code-panel';panel.className='sheet hidden';panel.innerHTML='<h3 id="module-code-title"></h3><label>规则代码<textarea id="module-code" spellcheck="false"></textarea></label><label>研究依据与固定测试<textarea id="module-cases" spellcheck="false"></textarea></label><button class="primary" id="save-module-code">测试并保存计算规则</button><p id="module-code-status"></p>';q('.module-body').append(panel);q('#save-module-code').onclick=async()=>{try{if(!q('#draft-id').value)throw new Error('请先复制版本');const v=await api('save_module_draft',wb.selectedModule,q('#module-code').value,q('#module-cases').value,q('#draft-id').value);q('#module-code-status').textContent=`${v.case_count}项测试通过，已保存`;notice('计算规则已保存到草稿')}catch(e){q('#module-code-status').textContent=e.message}};qa('[data-module]').forEach(button=>{const previous=button.onclick;button.onclick=async()=>{previous?.();const id=button.dataset.module;const module=skillModuleIds[id]||(id==='screening'?'screening':null);q('#module-code-panel').classList.toggle('hidden',!module);if(module){wb.selectedModule=module;const [author,version]=q('#edit-version').value.split('/');try{const doc=await api('get_module_document',module,version,author,q('#draft-id').value);q('#module-code-title').textContent=moduleName(module)+' · 计算规则';q('#module-code').value=doc.script;q('#module-cases').value=doc.cases;panel.scrollIntoView({block:'start',behavior:'smooth'})}catch(e){notice(e.message,true)}}}});};
